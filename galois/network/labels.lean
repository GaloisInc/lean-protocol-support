...
inductive agent_label : Type
  ...
  | send_message : remote_name → message_type → agent_label
...
def receives_message (m : message_type) : agent_label → Prop :=
