...
parameter {agents : map ip (agent message_type)}

instance decidable_agent_does (a_ip : ip) {P}
  [decidable_pred P] : decidable_pred (@agent_does message_type a_ip P)
:= begin 
...
inductive next_state_from_label_ind' (ag : agents.member) (s : system_state message_type ) (la : @agent_label message_type ) (s' : system_state message_type) : Prop
| mk : ∀ (next' : act message_type ag.value.state_type) (updatef : global_state_t message_type → global_state_t message_type)
       (Hagl : next_agent_state_from_label message_type ag.key ag.value (s.local_state ag) ((s.global_state ag.key)) la = some (next', updatef))
       (Hupd : s' = {local_state := lookup_update ag next' (s.local_state)
                    , global_state := updatef (s.global_state)})
      , next_state_from_label_ind'


inductive next_state_from_label_ind (a_ip : ip) (s : system_state message_type) (la : agent_label) (s' : system_state message_type) : Prop
...
lemma agent_does_invert {a_ip : ip} {P : @agent_label message_type → Prop}
  {l : next_state_label}
...
def fairness_spec : @TP message_type _ agents
...
lemma agent_update_invert' {tr : @TR message_type _ _} (ag : agents.member)
  (validtr : valid_trace LTS tr)
  {n : nat}
  {la : agent_label}
...
/-- Indicates that an agent is at the beginning of running 
    ...
    to that) 
-/
def starts_loop {a : agents.member} (next : act message_type a.value.state_type) : Prop :=
  ∃ (s : a.value.state_type), next = a.value.loop s

lemma return_implies_starts_loop (a : agents.member) :
  ∀ {s s' : system_state message_type} {l : @next_state_label message_type},
    LTS s l s'
...
def inLocalState (a : agents.member) (P : act message_type a.value.state_type → Prop)
  : system_state message_type × @next_state_label message_type → Prop
...
/- It feels like it should be possible to auto-generate something like
   ...
   from x to y)
-/
inductive strictly_smaller {A : Type u} (a : act message_type A) : act message_type A → Prop
| connect : ∀ (rn : remote_name) (cont : socket → act message_type A)
              (s : socket) (H : cont s = a), strictly_smaller (act.connect rn cont)
| send_message : ∀ (s : socket) (m : message_type) (cont : act _ A)
                (H : cont = a), strictly_smaller (act.send_message s m cont)
| poll : ∀ (ports : list port) (sockets : list socket) (bound : time)
      (cont : poll_result  ports sockets bound → act _ A)
...
lemma size_decreases {ag : agents.member} (a_ip : ip)
  {la : @agent_label message_type}
  {next' : act _ ag.value.state_type}
  {updatef : global_state_t message_type → global_state_t message_type}
  {s : system_state _}
  (H : next_agent_state_from_label _ a_ip ag.value (s.local_state ag)
...
{ cases (s.local_state ag); 
  ...
  { apply (if He : a < bound then _ else _),
    { rw (dif_pos He) at H,
      ...
      { -- new message
        cases ((list.member_st_decide 
          (λ (p : socket × message_type), p.snd = a_3) ((s.global_state a_ip).messages)));
          dsimp [next_agent_poll_state_from_label] at H;
          try { contradiction },
...
lemma local_state_stays_constant_ltl (a : agents.member)
  (P : act _ a.value.state_type → Prop)
  : ⊩ valid_trace LTS
...
apply_instance,
intros,
have H := local_state_stays_constant _ _ a_1 a_2,
rw ← H, assumption,
end
...
lemma agent_advances_helper2
  (a : agents.member)
  (ag_next : act _ a.value.state_type)
  : ⊩ valid_trace LTS
...
have H := local_state_stays_constant_ltl _ _ _ _ _ _ Hnow (trfair a n),
...
lemma inLocalState_meas_same (a : agents.member) (ag_next : act _ a.value.state_type)
  : inLocalState a (λ s, ag_next = s) 
  = λ s, (λ s' : system_state _ × next_state_label, s'.fst.local_state a) s = ag_next
:= begin
...
apply agent_advances_helper _ a _ tr trvalid trfair n rfl,
end

/-- If an agent always eventually polls, and if it is sent a message,
    then it eventually receives that message.
-/
def message_fairness_spec : @TP _ _ agents := λ tr,
  ∀ (a : agents.member) (mess : (socket × message_type)),
    (fair (now (inLabel (agent_does a.key polls))) 
     => □ (now (inState (λ s : system_state _, mess ∈ (s.global_state a.key).messages))
...
parameter ( message_type : Type)
parameter [decidable_eq message_type]
inductive can_possibly_step (a_ip : ip) (a : agent message_type) 
  (s : act message_type a.state_type) (l : @next_state_label message_type) (s' : act message_type a.state_type) : Prop
| mk : ∀ 
  (agents : map ip (@agent _ _))
  (Hmap : mapd.find a_ip agents = some a)
  (ss ss' : @system_state _ _ agents)
  (loc : ss.local_state ⟨ _, _, Hmap ⟩ = s)
  (loc' : ss'.local_state ⟨ _, _, Hmap ⟩ = s')
  (HLTS : LTS ss l ss'),
  can_possibly_step

inductive will_poll {a : agent message_type} : act message_type a.state_type → Prop
| mk : ∀ 
  (ports : list port)
  (sockets : list socket)
  (bound : time)
  (cont : poll_result ports sockets bound → act _ a.state_type),
  will_poll (act.poll ports sockets bound cont)

def agent_in_state_blocks_until_not_functional (a_ip : ip) 
  (a : agent _)
  (P : @poll_receive_label message_type → Prop) Q :=
...
-/
theorem blocking_agent_eventually_receives_message {agents : map ip (@agent _ _)}
  (a : agents.member) 
  (s : act _ a.value.state_type) (mess : message_type)
  (Hblock : agent_in_state_blocks_until_not a.key a.value s 
      (eq (poll_receive_label.receive_message mess)))
  : ⊩ valid_trace (@LTS _ _ agents)
    => fairness_spec _
    => message_fairness_spec _
    => now (inLocalState _ a (eq s))
    => now (inState (λ ss : system_state _, 
