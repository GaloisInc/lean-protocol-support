/- In this file, we define the core procedure behind
...
/-- To eliminate a variable from a focused problem,
    we take the upper and
    lower bounds to form new inequalities with
    `bounds_list`, and we leave the inequalities that
    didn't mention that variable in place
-/
def eliminate_aux {n : ℕ} (prob : focused n)
  : list (linear_expr n)
:= bounds_list prob ++ prob.uninvolved

/-- To eliminate a variable, we "focus" on that variable
    and then use the elimination procedure.
-/
...
/-- Any non-empty list has a minimal element when
    projected to a linearly-ordered type
-/
...
/-- For any non-empty list, we can find a value in the list
    whose value is equal to the minimum under a function `f`
    to a linearly-ordered type.
-/
...
/-- A lemma that says that if we can find a satisfying assignment
   for a pair of upper and lower bounds on a particular variable,
   that assignment (discarding the assignment to the particular variable)
   also satisfies the inequality generated by the `combine` function
   that is supposed to eliminate the variable.
-/
...
/-- The main theorem: If we have a focused problem and eliminate
    the focused variable, the new problem is satisfiable exactly
    when the old one was.
-/
theorem eliminate_aux_preserves_sat {n : ℕ} (prob : focused n)
...
/-- Another main theorem. The variable elimination procedure
...
/-- Deciding satisfiability of a problem with no
    variables in it is easy, since there is one
    possible assignment (the empty vector), so we
...
/-- Eliminate all of the variables from a
...
/-- If we eliminate all variables from a system of
    inequalities, we preserve satisfiability. This
    is just the repeated application of
    `satisfiable_eliminate`, which says that eliminating
    a single variable preserves satisfiability.
-/
...
/-- To decide satisfiability of a problem, we eliminate
    all the variables, at which point it is trivial.
-/
...
/-- The `decidable` instance given by `satisfiable_bool` returns
    the same value as a more concretely given Boolean function
    `satisfiable_bool'`. For some reason, I have difficulty getting
    the former to reduce, so I use the latter instead.
-/
